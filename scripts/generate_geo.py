...
region_items = [{"name": r, "slug": slugify(r)} for r in regions]
...
dpt_items = [{"code": code, "name": name, "slug": slugify(name)} for code, name in dpts]

out_dir = root / "src" / "data" / "geo"
out_dir.mkdir(parents=True, exist_ok=True)

with open(out_dir / "nation.json", "w", encoding="utf-8") as f:
    json.dump({"slug":"france","name":"France"}, f, ensure_ascii=False, indent=2)

with open(out_dir / "regions.json", "w", encoding="utf-8") as f:
    json.dump(region_items, f, ensure_ascii=False, indent=2)

with open(out_dir / "departements.json", "w", encoding="utf-8") as f:
    json.dump(dpt_items, f, ensure_ascii=False, indent=2)
